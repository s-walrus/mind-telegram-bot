...
class Game:
    ...
    def add_player(self, player_id):
        ...
        if self.status == NOT_STARTED:
            if player_id in self.player_hands.keys():
                response = WARNING
                throw_warning(
                    "The player has been already added to the game. "
            ...
            response = WARNING
            throw_warning(
                "A player was added after the game was started. "
    ...
    def act(self, player_id):
        ...
        if (
            ...
            and self.player_hands[player_id]
        ):
            ...
            for player_id in self.player_hands.keys():
                ...
                if new_hand != self.player_hands[player_id]:
                    ...
                    discarded[player_id] = [
                        item
                        for item in self.player_hands[player_id]
                        if item not in new_hand
                    ]
            ...
            if flag:
                ...
                if self.hp < 0:
                    self.status = LOSE
            if sum(map(sum, self.player_hands.values())) == 0 and self.hp >= 0:
                self.__finish_level()
        else:
            response = WARNING
            throw_warning(
    ...
    def release_hand(self, player_id):
        ...
        if (
            ...
            and self.player_status[player_id] == STOP
        ):
            self.player_status[player_id] = NORMAL
            ...
            player_id in self.player_hands.keys()
            and self.player_status[player_id] == SHURIKEN
        ):
            self.player_status[player_id] = NORMAL
        else:
            response = WARNING
    ...
    def vote_shuriken(self, player_id):
        ...
        if (
            ...
            and self.n_shurikens > 0
        ):
            self.player_status[player_id] = SHURIKEN
            if list(self.player_status.values()).count(SHURIKEN) == len(
                self.player_status.values()
            ):
            ...
            response = WARNING
            throw_warning(
                "Something went wrong during voting for shuriken. "
                "This call is ignored."
            )
        return self.__get_status(response, discarded=discarded)
    ...
    def __finish_level(self):
        if self.status == ACTION or self.status == CONCENTRATION:
    ...
    def __get_status(self, response, discarded=dict()):
        return {
            ...
            "level": self.level,
            "discarded": {
                player_id: discarded.get(player_id, [])
                for player_id in self.player_status.keys()
            },
        }
